...
.DESCRIPTION
    This script can be used in conjunction with Solarwinds TFTP server 
...
Requirements: 
  ...
  * The variable $tftpServerIP (declared in the script) must be set to the IP address of the computer runing the TFTP server.
      This will typically need to be changed if the script and the TFTP server are running on a different computer. 
...
function Backup-FirewallConfig {
    ...
    $myFunctions = @(
        ...
        "$PSScriptRoot\Other\StartStop-SolarwindsTftpServer.ps1"
        )
    ...
    $pwdFile = "$PSScriptRoot\Other\348514607"
    ...
    $encryptionKeyFile = "$PSScriptRoot\Other\1143630134"
